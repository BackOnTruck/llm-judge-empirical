...
Here is a readable summary, capturing the code's key functionality:
```
{}
```

An alternative readable summary that captures the code's key functionality can be:
```
"""
...
def obtain_score(lang: str, context: str, pred: str, ref: str, index: int, bar: tqdm):
    ...
    ref_pred = gptscore(template.format(context, ref), pred) # ref->pred
    pred_ref = gptscore(template.format(context, pred), ref) # pred->ref
