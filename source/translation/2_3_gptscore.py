...
Here is a readable and idiomatic {} translation, consistent with the original code's functionality:
```
{}
```

An alternative readable, idiomatic {} translation that is consistent with the original code can be:
```
"""
...
def obtain_score(lang: str, context: str, pred: str, ref: str, index: int, bar: tqdm):
    ...
    ref_pred = gptscore(template.format(src_lang, context, tgt_lang, ref, tgt_lang), pred) # ref->pred
    pred_ref = gptscore(template.format(src_lang, context, tgt_lang, pred, tgt_lang), ref) # pred->ref
