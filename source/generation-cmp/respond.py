from config import DIR, AUGMENTED_DATA, RESPONDED_DATA, MODELS, maxlen_resp, RESP_PER_INST
import json, os
...
infer_template = """Given the following requirement:
```
{}
```
...
def main():
    for sampled_data, responded_data in zip(AUGMENTED_DATA, RESPONDED_DATA):
        ...
        with open(f'{DIR}/{sampled_data}') as fin:
            for instance, models in [[json.loads(line), sample(MODELS, RESP_PER_INST)] for line in fin]:
                #! varies per task
                lang, text = instance['lang'], instance['input'].strip()
                prompt = infer_template.format(text, lang)
