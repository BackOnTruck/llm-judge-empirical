# modified from https://github.com/AIPHES/emnlp19-moverscore/blob/master/moverscore_v2.py
import numpy as np
import torch
import string
import os
...
device = 'cuda'
model_name = 'model/unixcoder-base'
...
def process(a):
    a = [tokenizer.cls_token, '<encoder-only>', tokenizer.sep_token]+truncate(tokenizer.tokenize(a))+[tokenizer.sep_token]
...
def collate_idf(arr, tokenize, numericalize, idf_dict,
    ...
    tokens = [[tokenizer.cls_token, '<encoder-only>', tokenizer.sep_token]+truncate(tokenize(a))+[tokenizer.sep_token] for a in arr]
...
def get_bert_embedding(all_sens, model, tokenizer, idf_dict,
    ...
    embeddings = []
    with torch.no_grad():
        for i in trange(0, len(all_sens), batch_size, desc='>>> Retrieving representations'):
...
def word_mover_score(refs, hyps, idf_dict_ref, idf_dict_hyp, stop_words=[], batch_size=128, device='cuda:0'):
    preds = []
    for batch_start in trange(0, len(refs), batch_size, desc='>>> Invoking model...'):
        batch_refs = refs[batch_start:batch_start+batch_size]
        batch_hyps = hyps[batch_start:batch_start+batch_size]

        ref_embedding, _, _, ref_idf, ref_tokens = get_bert_embedding(batch_refs, model, tokenizer, idf_dict_ref,device=device)
        hyp_embedding, _, _, hyp_idf, hyp_tokens = get_bert_embedding(batch_hyps, model, tokenizer, idf_dict_hyp,device=device)
        ...
        for i in trange(batch_size, desc='>>> Calculating flow'):
...
# end of moverscore_v2.py

from config import DIR, RESPONDED_DATA, MS_RES
import json

def main():
    with open(f'{DIR}/{RESPONDED_DATA[0]}') as fin, open(f'{DIR}/{MS_RES}', 'w') as fout:
        refs, preds = [], []
        for line in fin:
            js = json.loads(line)
            refs += [js['gold']]
        ...
        idf_ref, idf_pred = map(get_idf_dict, [refs, preds])
        for score in word_mover_score(refs, preds, idf_ref, idf_pred):
