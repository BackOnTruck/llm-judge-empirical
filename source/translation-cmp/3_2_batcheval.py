...
from config import gpt4o, RESPONDED_DATA, DIR, BATCH_RES, CRITERIA, maxlen_output
...
def get_score(response: str, num: int):
    ...
    if len(s) != num and len(s) != 5:
        ...
        #! judgments for code translation might be rather long; we declare draw when no valid score is detected
...
Each sample contains an original code snippet, and two translations in another programming language.
You are to compare the quality of the translations, without considering the quality of the original code, and decide which one is noticeably better or declare a tie because their difference in quality is insignificant.
...
def solve(batch: List[Dict[str, Union[str, float]]], bar: tqdm):
    ...
    for i, sample in enumerate(batch, start=1):
        src_lang, tgt_lang = sample['lang'].split(', ')
        #! params vary per task
        contexts += template_per_sample.format(i, src_lang, sample['input'], tgt_lang, sample['out1'], tgt_lang, sample['out2'])

    try:
        scores = get_score(Model.generate(template.format(contexts), T=T, maxlen=maxlen_output * 2)[0], len(batch))
        #! we made an exception to use longer context length for code translation
        ...
        for i in range(0, len(lines), 3):
            ...
            for j in range(3):
                for k in range(3):
                    if j != k:
                        #! params vary per task
                        inp, out1, out2, lang = samples_3[0]['input'], samples_3[j]['output'], samples_3[k]['output'], samples_3[k]['lang']
                        samples += [{'input': inp, 'out1': out1, 'out2': out2, 'score': 0.0, 'index': index, 'lang': lang}]
                        index += 1
